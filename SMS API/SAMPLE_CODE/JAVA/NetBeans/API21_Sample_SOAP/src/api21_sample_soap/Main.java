...
public class Main {
    ...
    public static void main(String[] args) {
		...
		String userID="0963139051";	//帳號
		String password="1qaz";	//密碼
		String subject="測試API2.1";	//簡訊主旨，主旨不會隨著簡訊內容發送出去。用以註記本次發送之用途。可傳入空字串。
		String content="測試API2.1 JAVA(NetBeans)";	//簡訊發送內容
		...
		try{
			if(sms.getConnection(userID, password)){
				System.out.println(new StringBuffer("取得連線成功，SessionKey：").append(sms.getSessionKey()).toString());
				if(sms.getCredit()){
				...
				System.out.println(new StringBuffer("取得連線失敗，失敗原因：").append(sms.getProcessMsg()).toString());
			}
		}catch(Exception ex){
